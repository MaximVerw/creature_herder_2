...
public class FoodPoop extends RenderableObject {
  ...
  public FoodPoop(Vector2 worldCoord, Food food, FoodDispenser dump, boolean withOffset, boolean skipStage1, Optional<Vector2> overrideHoming) {
    renderable =
        new Renderable(
            food.getTexture(),
            worldCoord,
  ...
  public void update(float delta) {
    ...
    if (stage1) {
      if (targetCoord.dst(renderable.getWorldCoord()) > 0.005f*speed) {
        renderable.setWoordCoord(softLerp(renderable.getWorldCoord(), targetCoord, delta*speed, 0.005f*speed));
        renderable.size =
            new Vector2(
